...
public class FlowTagLayout extends ViewGroup {
    ...
    private boolean isMultLinesShow = false;// default state
    public View mView;

    public boolean isMultLinesShow() {
        return isMultLinesShow;
    }
    ...
    public void setMultLinesShow(boolean multLinesShow) {
        isMultLinesShow = multLinesShow;
        requestLayout();
        //invalidate();
    }

    public boolean getMultLinesShow() {
        return isMultLinesShow;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0, childCount = getChildCount(); i < childCount; i++) {
            ...
            if (mView != null) {
                if ((resultLineWidth + realChildWidth) > sizeWidth) {
            ...
            if (isMultLinesShow && (lineWidth + realChildWidth) > sizeWidth) {
                //换行
                ...
                resultLineWidth += realChildWidth;
                //每一行的高度取二者最大值
...
// 这个构造方法所然能使用但是编译器会报错
                addView(childView, new MarginLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT,
                    ...
                    if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
                        ...
                        if (mCheckedTagArray.get(j)) {
                            ...
                            if (mOnTagSelectListener != null) {
                                mOnTagSelectListener.onItemSelect(FlowTagLayout.this, new
                                        ArrayList<Integer>());
                            }
                            return;
                        }
