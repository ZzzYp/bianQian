package com.gome.note.camera.adapter;
...
//                    if (listener != null) listener.onImageItemClick(rootView, imageItem, position);
                    ...
                    if (cbCheck.isChecked() && mSelectedImages.size() >= selectLimit) {
                        if (toast == null) {
                            toast = Toast.makeText(mActivity.getApplicationContext(), mActivity.getString(R.string.ip_select_limit, selectLimit), Toast.LENGTH_SHORT);
                        }
                        toast.show();
